...
bool Camera::get_capability() {
  ...
  if (xioctl(static_cast<int>(VIDIOC_QUERYCAP), &v4l2_s.capability) == -1) {
...
void Camera::check_essential_capability() {
  if (!(v4l2_s.capability.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
    ...
    throw std::runtime_error("V4L2_CAP_VIDEO_CAPTURE not supported");
  }

  if (!(v4l2_s.capability.capabilities & V4L2_CAP_STREAMING)) {
    DBG_PERROR("V4L2_CAP_STREAMING");
    throw std::runtime_error("V4L2_CAP_STREAMING not supported");
  }
}
...
bool Camera::set_buffer() {
  ...
  if (xioctl(static_cast<int>(VIDIOC_REQBUFS), &v4l2_s.requestbuffers) == -1) {
  ...
  for (unsigned int i = 0; i < buffer_count; i++) {
    ...
    if (xioctl(static_cast<int>(VIDIOC_QUERYBUF), &v4l2_s.buffer) == -1) {
...
bool Camera::enumerate_image_formats(const enum v4l2_buf_type buf_type) {
  ...
  while (xioctl(static_cast<int>(VIDIOC_ENUM_FMT), &v4l2_s.fmtdesc) != -1) {
    DBG_PRINTF("Index: %d", v4l2_s.fmtdesc.index);
    DBG_PRINTF("Format description: %s", v4l2_s.fmtdesc.description);
    DBG_PRINTF("Format pixel format: %c, %c, %c, %c", (v4l2_s.fmtdesc.pixelformat >> 0) & 0xFF, (v4l2_s.fmtdesc.pixelformat >> 8) & 0xFF, (v4l2_s.fmtdesc.pixelformat >> 16) & 0xff,
...
bool Camera::enumerate_frame_sizes(const unsigned int pixelformat, std::string& description) {
  ...
  while (xioctl(static_cast<int>(VIDIOC_ENUM_FRAMESIZES), &v4l2_s.frmsizeenum) != -1) {
    ...
    switch (v4l2_s.frmsizeenum.type) {
      ...
      case V4L2_FRMSIZE_TYPE_STEPWISE:
        DBG_PRINTF("Stepwise :: Width step(max/min): %d (%d / %d), Height step(max / min): %d (%d / %d)", v4l2_s.frmsizeenum.stepwise.step_width, v4l2_s.frmsizeenum.stepwise.max_width,
                   v4l2_s.frmsizeenum.stepwise.min_width, v4l2_s.frmsizeenum.stepwise.step_height, v4l2_s.frmsizeenum.stepwise.max_height, v4l2_s.frmsizeenum.stepwise.min_height);
        break;
...
bool Camera::enumerate_frame_intervals(const unsigned int pixelformat, const unsigned int width, const unsigned int height, std::string& description) {
  ...
  while (xioctl(static_cast<int>(VIDIOC_ENUM_FRAMEINTERVALS), &v4l2_s.frmivalenum) != -1) {
    switch (v4l2_s.frmivalenum.type) {
      ...
      case V4L2_FRMIVAL_TYPE_STEPWISE:
        DBG_PRINTF("Stepwise step (max/min): %.2f (%.2f / %.2f) fps", 1.0 / ((double)v4l2_s.frmivalenum.stepwise.step.numerator / (double)v4l2_s.frmivalenum.stepwise.step.denominator),
...
int Camera::enumerate_controls() {
  ...
  while (query_ioctl(static_cast<int>(VIDIOC_QUERYCTRL), &v4l2_s.queryctrl) == 0) {
...
int Camera::enumerate_control_menu() {
  ...
  for (v4l2_s.querymenu.index = v4l2_s.queryctrl.minimum; (int)v4l2_s.querymenu.index <= v4l2_s.queryctrl.maximum; v4l2_s.querymenu.index++) {
    if (xioctl(static_cast<int>(VIDIOC_QUERYMENU), &v4l2_s.querymenu) == 0) {
...
bool Camera::start() {
  ...
  for (unsigned int i = 0; i < buffer_count; i++) {
    ...
    v4l2_s.buffer.memory = V4L2_MEMORY_MMAP;
    v4l2_s.buffer.index = i;
    if (xioctl(static_cast<int>(VIDIOC_QBUF), &v4l2_s.buffer) == -1) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_STREAMON), &v4l2_s.buf_type) == -1) {
...
bool Camera::stop() {
  ...
  v4l2_s.buf_type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  if (xioctl(static_cast<int>(VIDIOC_STREAMOFF), &v4l2_s.buf_type) == -1) {
...
bool Camera::get_control(camera_control& ctrl) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_QUERYCTRL), &v4l2_s.queryctrl) == -1) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_G_CTRL), &control) == -1) {
...
bool Camera::set_control(const char* name, const int value) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_S_CTRL), &control) == -1 && errno != ERANGE) {
...
bool Camera::get_current_format() {
  ...
  if (xioctl(static_cast<int>(VIDIOC_G_FMT), &v4l2_s.format.type) == -1) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_G_PARM), &v4l2_s.streamparm) == -1) {
...
bool Camera::set_format(unsigned int width, unsigned int height, unsigned int pixelformat, unsigned int rate_numerator, unsigned int rate_denomonator) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_S_FMT), &v4l2_s.format) == -1) {
  ...
  if (xioctl(static_cast<int>(VIDIOC_S_PARM), &v4l2_s.streamparm) == -1) {
...
int Camera::get_buffer(unsigned char* buffer, const unsigned int size) {
  ...
  v4l2_s.buffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  v4l2_s.buffer.memory = V4L2_MEMORY_MMAP;
  if (xioctl(static_cast<int>(VIDIOC_DQBUF), &v4l2_s.buffer) == -1) {
